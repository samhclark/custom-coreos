...
| Field | Value |
| --- | --- |
| Overall status | Phases 2 through 5 are complete; the Phase 6 VictoriaMetrics krun change is locally validated but not deployed |
| Last completed work | 2026-08-02: verified `tank/victoria-metrics/data@pre-krun-20260802-220254`; all Phase 6 baseline, local-validation, and rollback-preparation gates are complete |
| Current phase | Phase 6: VictoriaMetrics krun change is ready for scheduled deployment |
| Next concrete action | Push local `main` when ready for the scheduled image build and NAS update; afterward, prove krun runtime, historical continuity, fresh ingestion, dependency health, compaction, and resources |
...
| Last NAS validation | 2026-08-02: `tank/victoria-metrics/data@pre-krun-20260802-220254` was verified at 1.32 MiB referenced snapshot space; ordinary-crun VictoriaMetrics remained healthy after the snapshot restart |
