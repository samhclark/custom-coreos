...
# Exposing buckets as websites {#exposing-buckets-as-websites .title .leading-10 .text-xl .lg:text-2xl .xl:text-3xl .text-garage-orange .font-semibold}
...
1.  using the PutBucketWebsite S3 API call, which is allowed for access
    keys that have the owner permission bit set

2.  from the Garage CLI, by an adminstrator of the cluster

3.  using the Garage administration API

The `PutBucketWebsite` API endpoint [is
documented](https://docs.aws.amazon.com/AmazonS3/latest/API/API_PutBucketWebsite.html){rel="noopener nofollow noreferrer"
target="_blank"} in the official AWS docs. This endpoint can also be
called [using
`aws s3api`](https://docs.aws.amazon.com/cli/latest/reference/s3api/put-bucket-website.html){rel="noopener nofollow noreferrer"
target="_blank"} on the command line. The website configuration
supported by Garage is only a subset of the possibilities on Amazon S3:
redirections are not supported, only the index document and error
document can be specified.

If you want to expose your bucket as a website from the CLI, use this
simple command:
...
Now it will be **publicly** exposed on the web endpoint (by default
...
- The index file is defined per-bucket and can be specified in the
  `PutBucketWebsite` call or on the CLI using the `--index-document`
  parameter (default: `index.html`)
- A custom error document for 404 errors can be specified in the
  `PutBucketWebsite` call or on the CLI using the `--error-document`
  parameter

Now we need to infer the URL of your website through your bucket name.
Let assume:

- we set `root_domain = ".web.example.com"` in `garage.toml`
  ([ref](https://garagehq.deuxfleurs.fr/documentation/reference-manual/configuration/#web_root_domain))
- our bucket name is `garagehq.deuxfleurs.fr`.

Our bucket will be served if the Host field matches one of these 2
values (the port is ignored):

- `garagehq.deuxfleurs.fr.web.example.com`: you can dedicate a subdomain
  to your users (here `web.example.com`).

- `garagehq.deuxfleurs.fr`: your users can bring their own domain name,
  they just need to point them to your Garage cluster.
...
- or configure a [reverse
  proxy](https://garagehq.deuxfleurs.fr/documentation/cookbook/reverse-proxy/)
  in front of Garage to add TLS (HTTPS), CORS support, etc.

You can also take a look at [Website
Integration](https://garagehq.deuxfleurs.fr/documentation/connect/websites/)
to see how you can add Garage to your workflow.
:::
::::::
...
1.  [Configuring a bucket for website
    access](https://garagehq.deuxfleurs.fr/documentation/cookbook/exposing-websites/#configuring-a-bucket-for-website-access){.font-semibold
    .text-gray-800 .transition-all .hover:text-garage-orange}
2.  [How exposed websites
    work](https://garagehq.deuxfleurs.fr/documentation/cookbook/exposing-websites/#how-exposed-websites-work){.font-semibold
    .text-gray-800 .transition-all .hover:text-garage-orange}
