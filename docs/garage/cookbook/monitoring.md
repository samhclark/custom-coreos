...
# Monitoring Garage {#monitoring-garage .title .leading-10 .text-xl .lg:text-2xl .xl:text-3xl .text-garage-orange .font-semibold}
...
Garage exposes some internal metrics in the Prometheus data format. This
page explains how to exploit these metrics.

## Setting up monitoring

### Enabling the Admin API endpoint

If you have not already enabled the [administration API
endpoint](https://garagehq.deuxfleurs.fr/documentation/reference-manual/admin-api/),
do so by adding the following lines to your configuration file:
...
`http://localhost:3903/metrics`. If you want to restrict access to the
exported metrics, set the `metrics_token` configuration value to a
bearer token to be used when fetching the metrics endpoint.

### Setting up Prometheus and Grafana

Add a scrape config to your Prometheus daemon to scrape metrics from all
of your nodes:

``` {.language-yaml data-lang="yaml" style="background-color:#2b303b;color:#c0c5ce;"}
scrape_configs:
  - job_name: 'garage'
    static_configs:
      - targets:
...
If you have set a metrics token in your Garage configuration file, add
the following lines in your Prometheus scrape config:
...
To visualize the scraped data in Grafana, you can either import our
[Grafana dashboard for
Garage](https://git.deuxfleurs.fr/Deuxfleurs/garage/raw/branch/main/script/telemetry/grafana-garage-dashboard-prometheus.json){rel="noopener nofollow noreferrer"
target="_blank"} or make your own.

The list of exported metrics is available on our [dedicated
page](https://garagehq.deuxfleurs.fr/documentation/reference-manual/monitoring/)
...
1.  [Setting up
    monitoring](https://garagehq.deuxfleurs.fr/documentation/cookbook/monitoring/#setting-up-monitoring){.font-semibold
    .text-gray-800 .transition-all .hover:text-garage-orange}
    - [Enabling the Admin API
      endpoint](https://garagehq.deuxfleurs.fr/documentation/cookbook/monitoring/#enabling-the-admin-api-endpoint){#link-enabling-the-admin-api-endpoint
      .toc .text-gray-700 .hover:text-garage-orange}
    - [Setting up Prometheus and
      Grafana](https://garagehq.deuxfleurs.fr/documentation/cookbook/monitoring/#setting-up-prometheus-and-grafana){#link-setting-up-prometheus-and-grafana
      .toc .text-gray-700 .hover:text-garage-orange}
