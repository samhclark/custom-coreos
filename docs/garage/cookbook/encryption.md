...
# Encryption {#encryption .title .leading-10 .text-xl .lg:text-2xl .xl:text-3xl .text-garage-orange .font-semibold}
...
Encryption is a recurring subject when discussing Garage. Garage does
not handle data encryption by itself, but many things can already be
done with Garage\'s current feature set and the existing ecosystem.

This page takes a high level approach to security in general and data
...
This page explains what Garage provides, and how you can improve the
situation by yourself by adding encryption at different levels.

We would be very curious to know your needs and thougs about ideas such
as encryption practices and things like key management, as we want
Garage to be a serious base platform for the developpment of secure,
encrypted applications. Do not hesitate to come talk to us if you have
...
RPCs between Garage nodes are encrypted. More specifically, contrary to
many distributed software, it is impossible in Garage to have clear-text
RPC. We use the [kuska
handshake](https://github.com/Kuska-ssb/handshake){rel="noopener nofollow noreferrer"
target="_blank"} library which implements a protocol that has been
clearly reviewed, Secure ScuttleButt\'s Secret Handshake protocol. This
is why setting a `rpc_secret` is mandatory, and that\'s also why your
nodes have super long identifiers.
...
For standard S3 API requests, Garage does not encrypt data at rest by
itself. For the most generic at rest encryption of data, we recommend
setting up your storage partitions on encrypted LUKS devices.

If you are developping your own client software that makes use of S3
storage, we recommend implementing data encryption directly on the
client side and never transmitting plaintext data to Garage. This makes
it easy to use an external untrusted storage provider if necessary.

Garage does support [SSE-C
encryption](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ServerSideEncryptionCustomerKeys.html){rel="noopener nofollow noreferrer"
target="_blank"}, an encryption mode of Amazon S3 where data is
encrypted at rest using encryption keys given by the client. The
encryption keys are passed to the server in a header in each request, to
encrypt or decrypt data at the moment of reading or writing. The server
discards the key as soon as it has finished using it for the request.
This mode allows the data to be encrypted at rest by Garage itself, but
it requires support in the client software. It is also not adapted to a
model where the server is not trusted or assumed to be compromised, as
the server can easily know the encryption keys. Note however that when
using SSE-C encryption, the only Garage node that knows the encryption
key passed in a given request is the node to which the request is
directed (which can be a gateway node), so it is easy to have untrusted
nodes in the cluster as long as S3 API requests containing SSE-C
encryption keys are not directed to them.

Implementing automatic data encryption directly in Garage without
client-side management of keys (something like
[SSE-S3](https://docs.aws.amazon.com/AmazonS3/latest/userguide/UsingServerSideEncryption.html){rel="noopener nofollow noreferrer"
target="_blank"}) could make things simpler for end users that don\'t
want to setup LUKS, but also raises many more questions, especially
around key management: for encryption of data, where could Garage get
the encryption keys from? If we encrypt data but keep the keys in a
plaintext file next to them, it\'s useless. We probably don\'t want to
have to manage secrets in Garage as it would be very hard to do in a
secure way. At the time of speaking, there are no plans to implement
this in Garage.
...
You have multiple options to have encryption between your client and a
node:

- Setup a reverse proxy with TLS / ACME / Let\'s encrypt
- Setup a Garage gateway locally, and only contact the garage daemon on
  `localhost`
- Only contact your Garage daemon over a secure, encrypted overlay
  network such as Wireguard
...
Crucially, does not protect againt malicious sysadmins or remote
attackers that might gain access to your servers.
...
- Matrix: uses the OLM protocol for E2EE of user messages. Media files
  stored in Matrix are probably encrypted using symmetric encryption,
  with a key that is distributed in the end-to-end encrypted message
  that contains the link to the object.

- XMPP: clients normally support either OMEMO / OpenPGP for the E2EE of
  user messages. Media files are encrypted per
  [XEP-0454](https://xmpp.org/extensions/xep-0454.html){rel="noopener nofollow noreferrer"
  target="_blank"}.

- Aerogramme: use the user\'s password as a key to decrypt data in the
  user\'s bucket

- Cyberduck: comes with support for
  [Cryptomator](https://docs.cyberduck.io/cryptomator/){rel="noopener nofollow noreferrer"
  target="_blank"} which allows users to create client-side vaults to
  encrypt files in before they are uploaded to a cloud storage endpoint.
:::
::::::
...
1.  [Examining your need for
    encryption](https://garagehq.deuxfleurs.fr/documentation/cookbook/encryption/#examining-your-need-for-encryption){.font-semibold
    .text-gray-800 .transition-all .hover:text-garage-orange}
2.  [Capabilities provided by
    Garage](https://garagehq.deuxfleurs.fr/documentation/cookbook/encryption/#capabilities-provided-by-garage){.font-semibold
    .text-gray-800 .transition-all .hover:text-garage-orange}
    - [Traffic is encrypted between Garage
      nodes](https://garagehq.deuxfleurs.fr/documentation/cookbook/encryption/#traffic-is-encrypted-between-garage-nodes){#link-traffic-is-encrypted-between-garage-nodes
      .toc .text-gray-700 .hover:text-garage-orange}
    - [HTTP API endpoints provided by Garage are in clear
      text](https://garagehq.deuxfleurs.fr/documentation/cookbook/encryption/#http-api-endpoints-provided-by-garage-are-in-clear-text){#link-http-api-endpoints-provided-by-garage-are-in-clear-text
      .toc .text-gray-700 .hover:text-garage-orange}
    - [Garage stores data in plain text on the filesystem or encrypted
      using customer keys
      (SSE-C)](https://garagehq.deuxfleurs.fr/documentation/cookbook/encryption/#garage-stores-data-in-plain-text-on-the-filesystem-or-encrypted-using-customer-keys-sse-c){#link-garage-stores-data-in-plain-text-on-the-filesystem-or-encrypted-using-customer-keys-sse-c
      .toc .text-gray-700 .hover:text-garage-orange}
3.  [Adding data encryption using external
    tools](https://garagehq.deuxfleurs.fr/documentation/cookbook/encryption/#adding-data-encryption-using-external-tools){.font-semibold
    .text-gray-800 .transition-all .hover:text-garage-orange}
    - [Encrypting traffic between a Garage node and your
      client](https://garagehq.deuxfleurs.fr/documentation/cookbook/encryption/#encrypting-traffic-between-a-garage-node-and-your-client){#link-encrypting-traffic-between-a-garage-node-and-your-client
      .toc .text-gray-700 .hover:text-garage-orange}
    - [Encrypting data at
      rest](https://garagehq.deuxfleurs.fr/documentation/cookbook/encryption/#encrypting-data-at-rest){#link-encrypting-data-at-rest
      .toc .text-gray-700 .hover:text-garage-orange}
    - [Encrypting data on the client
      side](https://garagehq.deuxfleurs.fr/documentation/cookbook/encryption/#encrypting-data-on-the-client-side){#link-encrypting-data-on-the-client-side
      .toc .text-gray-700 .hover:text-garage-orange}
...
.h-6 .mx-auto} [Cookbook]{.mt-0.5 .2xl:hidden}
]{.icon}](https://garagehq.deuxfleurs.fr/documentation/cookbook/ "Cookbook"){.flex
