...
GRAFANA_CONFIG_FCONTEXT="/usr/share/custom-coreos/grafana(/.*)?"
GRAFANA_DATA_FCONTEXT="/var/lib/grafana(/.*)?"
...
ensure_fcontext_rule "${GRAFANA_CONFIG_FCONTEXT}"
ensure_fcontext_rule "${GRAFANA_DATA_FCONTEXT}"

if [[ -e /var/lib/grafana ]]; then
    log "Restoring SELinux labels for /var/lib/grafana"
    restorecon -F -R /var/lib/grafana
fi
