...
- `caddy.container` - reverse proxy / TLS termination for the user-facing services; rootless under `etc/containers/systemd/users/51310/`, configured for libkrun with private nested passt networking after the earlier direct-TSI deployment exposed stream head-of-line blocking
...
- Caddy uses 2 vCPUs and 512 MiB under libkrun. Its outer rootless pasta namespace publishes TCP 80/443, UDP 443, and loopback-only TCP 2019; `-T` allowlists host-loopback backends 3000, 3900, 3903, 8096, and 8428. Inner crun passt supplies virtio-net and HTTP/3 while its broad `-t all -u all` listeners remain confined to Caddy's private namespace. Retain host `net.ipv4.ip_unprivileged_port_start=80` and the matching Caddy `Sysctl=`: new network namespaces do not inherit the host value, and inner passt otherwise cannot claim 80/443. The krun handler still lacks `podman exec`, so configuration changes use service restarts.
- Jellyfin uses a 4-vCPU, 4-GiB libkrun guest with nested passt and loopback-only host TCP 8096 behind Caddy. Its image healthcheck is disabled because it requires unsupported `podman exec`; blackbox probing is authoritative. The deployment deliberately omits `/dev/dri` hardware acceleration and UDP discovery; `/var/zfs/tank/videos/{movies,tv-shows}` is mounted read-only without Podman `:z`/`:Z` relabeling.
