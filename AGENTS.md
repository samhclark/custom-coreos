...
Current state:
- Thirteen image-defined services are deployed as rootless admin-managed user Quadlets: the existing ingress, observability, Garage, and Jellyfin services plus the four-component Immich application. Immich first use is validated; a reboot exposed and motivated the mapped-ownership storage-readiness fix, whose clean post-update reboot remains to be confirmed. Jellyfin's service path is operational, while representative playback and VM-isolated hardware transcoding remain active validation work.
...
- Owned storage roots require the exact service UID/GID. Descendants may use that identity or IDs inside the service's generated subordinate range; container-root-created mount anchors are not ownership drift. For descendant labels, enforce `object_r:container_file_t:s0` with no MCS categories rather than requiring a particular SELinux user field.
