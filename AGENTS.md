...
- Caddy uses 2 vCPUs and 512 MiB under libkrun. Its outer rootless pasta namespace publishes TCP 80/443, UDP 443, and loopback-only TCP 2019; `-T` allowlists host-loopback backends 3000, 3900, 3903, 8096, and 8428. Inner crun passt supplies virtio-net and HTTP/3 while its broad `-t all -u all` listeners remain confined to Caddy's private namespace. Retain `net.ipv4.ip_unprivileged_port_start=80`. The krun handler still lacks `podman exec`, so configuration changes use service restarts.
